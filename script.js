class HotezaApp {
    ...
    initContentPages() {
        ...
        this.showContentPage('floor-planner');
    }
...
// ==================== FLOOR PLANNER ====================
renderFloorPlanner() {
    const container = document.getElementById('floor-planner');
    if (!container) return;
    
    container.innerHTML = `
        <div class="content-header">
            <h2><i class="fas fa-vector-square"></i> Floor Planner</h2>
            <div class="breadcrumbs">
                <span>Home</span> <i class="fas fa-chevron-right"></i> <span>Floor Planner</span>
            </div>
        </div>

        <div class="floor-planner-container">
            <div class="planner-toolbar">
                <div class="tool-section">
                    <h4>Tables</h4>
                    <div class="tool-options">
                        <button class="btn-tool" data-tool="table" data-type="round">
                            <i class="fas fa-circle"></i> Round
                        </button>
                        <button class="btn-tool" data-tool="table" data-type="square">
                            <i class="fas fa-square"></i> Square
                        </button>
                        <button class="btn-tool" data-tool="table" data-type="rectangle">
                ...
                <div class="tool-section">
                    <h4>Walls</h4>
                    <div class="tool-options">
                        <button class="btn-tool" data-tool="wall" data-type="straight">
                            <i class="fas fa-grip-lines"></i> Straight
                        </button>
                        <button class="btn-tool" data-tool="wall" data-type="corner">
                            <i class="fas fa-vector-square"></i> Corner
                        </button>
                    </div>
                </div>
                ...
                <div class="tool-section">
                    <h4>Decorations</h4>
                    <div class="tool-options">
                        <button class="btn-tool" data-tool="decoration" data-type="plant">
                            <i class="fas fa-leaf"></i> Plant
                        </button>
                        <button class="btn-tool" data-tool="decoration" data-type="bar">
                            <i class="fas fa-glass-martini-alt"></i> Bar
                        </button>
                        <button class="btn-tool" data-tool="decoration" data-type="counter">
                            <i class="fas fa-utensils"></i> Counter
                        </button>
                    </div>
                </div>
                ...
                <div class="action-buttons">
                    ...
                    <button class="btn btn-outline" id="clear-layout">
                        <i class="fas fa-trash"></i> Clear
                    </button>
                </div>
            </div>

            <div class="planner-canvas" id="planner-canvas">
    ...
    // Initialize the floor planner
    this.initFloorPlanner();
}

initFloorPlanner() {
    // Load saved layout or initialize empty
    this.floorItems = JSON.parse(localStorage.getItem('hoteza_floorPlan')) || [];
    ...
    this.setupFloorPlannerEvents();
}

setupFloorPlannerEvents() {
    ...
    document.querySelectorAll('.btn-tool').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // For immediate placement tools
            if (this.selectedTool === 'decoration') {
                this.addFloorItem(this.selectedTool, type);
            }
        });
    });
    ...
    canvas.addEventListener('mousedown', (e) => {
        ...
        // For walls and tables that need dragging to create
        if (this.selectedTool === 'wall' || this.selectedTool === 'table') {
            const tempId = 'temp-' + Date.now();
            this.tempItem = {
                id: tempId,
                type: this.selectedTool,
                subtype: document.querySelector('.btn-tool.active').dataset.type,
                x: this.startPos.x,
                y: this.startPos.y,
                width: 0,
                height: 0,
            ...
            this.renderFloorItems([this.tempItem]);
        }
    });

    canvas.addEventListener('mousemove', (e) => {
        if (!this.isDrawing || !this.tempItem) return;
        ...
        // Update dimensions
        this.tempItem.width = currentPos.x - this.startPos.x;
        this.tempItem.height = currentPos.y - this.startPos.y;
        
        // For square tables, keep width = height
        if (this.tempItem.subtype === 'square') {
            const size = Math.max(Math.abs(this.tempItem.width), Math.abs(this.tempItem.height));
            this.tempItem.width = this.tempItem.width < 0 ? -size : size;
            this.tempItem.height = this.tempItem.height < 0 ? -size : size;
        }
        
        // For round tables, use diameter
        if (this.tempItem.subtype === 'round') {
            const diameter = Math.sqrt(
                Math.pow(this.tempItem.width, 2) + 
                Math.pow(this.tempItem.height, 2)
            );
            this.tempItem.width = diameter;
            this.tempItem.height = diameter;
        }
    ...
    canvas.addEventListener('mouseup', (e) => {
        ...
        if (this.tempItem) {
            ...
            if (Math.abs(this.tempItem.width) > 20 && Math.abs(this.tempItem.height) > 20) {
                this.tempItem.id = 'item-' + Date.now();
                this.floorItems.push(this.tempItem);
    ...
    // Item selection
    floorItems.addEventListener('click', (e) => {
        const itemElement = e.target.closest('.floor-item');
        if (itemElement) {
            const itemId = itemElement.dataset.id;
            this.selectedItem = this.floorItems.find(item => item.id === itemId);
            this.renderPropertyForm();
    ...
    document.getElementById('save-layout').addEventListener('click', () => {
        this.saveFloorPlan();
        this.showAlert('Floor plan saved successfully', 'success');
    });
...
renderFloorItems(items = this.floorItems) {
    ...
    items.forEach(item => {
        const itemEl = document.createElement('div');
        itemEl.className = `floor-item ${item.type} ${item.subtype} ${item.id === this.selectedItem?.id ? 'selected' : ''}`;
        ...
        if (item.type === 'table') {
            itemEl.style.width = `${Math.abs(item.width)}px`;
            itemEl.style.height = `${Math.abs(item.height)}px`;
            ...
            if (item.waiter) {
                const waiterEl = document.createElement('div');
                waiterEl.className = 'table-waiter';
                waiterEl.textContent = item.waiter;
                itemEl.appendChild(waiterEl);
            }
        } 
        else if (item.type === 'wall') {
            const length = Math.sqrt(item.width * item.width + item.height * item.height);
            const angle = Math.atan2(item.height, item.width) * 180 / Math.PI;
            
            itemEl.style.width = `${length}px`;
            itemEl.style.transform = `rotate(${angle}deg)`;
            itemEl.style.transformOrigin = '0 0';
        }
        else if (item.type === 'decoration') {
            itemEl.innerHTML = `<i class="fas fa-${this.getDecorationIcon(item.subtype)}"></i>`;
        }
    ...
    container.addEventListener('drop', (e) => {
        ...
        const item = this.floorItems.find(i => i.id === itemId);
        
        if (item) {
            const rect = container.getBoundingClientRect();
            item.x = e.clientX - rect.left - (item.width / 2);
            item.y = e.clientY - rect.top - (item.height / 2);
            this.saveFloorPlan();
            this.renderFloorItems();
        }
    });
...
renderPropertyForm() {
    ...
    if (!this.selectedItem) {
        container.innerHTML = '<div class="empty-state">Select an item to edit properties</div>';
        return;
    }
    ...
    if (this.selectedItem.type === 'table') {
        formHTML = `
            <div class="form-group">
                ...
                <input type="text" id="table-number" value="${this.selectedItem.number || ''}">
            </div>
            <div class="form-group">
                ...
                <select id="table-waiter">
                    ...
                    ${this.waiters.map(waiter => `
                        <option value="${waiter.name}" ${this.selectedItem.waiter === waiter.name ? 'selected' : ''}>
            ...
            <div class="form-group">
                ...
                <div class="size-controls">
                    <input type="number" id="table-width" value="${Math.abs(this.selectedItem.width)}" min="20">
                    <span>x</span>
                    <input type="number" id="table-height" value="${Math.abs(this.selectedItem.height)}" min="20">
                    <span>px</span>
                </div>
            </div>
            <button class="btn btn-primary" id="save-properties">Save</button>
            <button class="btn btn-outline" id="delete-item">Delete</button>
        `;
    }
        ...
        formHTML = `
            <div class="form-group">
                <label>Length</label>
                <input type="number" id="wall-length" value="${Math.sqrt(
                    this.selectedItem.width * this.selectedItem.width + 
                    this.selectedItem.height * this.selectedItem.height
                ).toFixed(0)}" min="20">
            </div>
            <div class="form-group">
                <label>Angle</label>
                <input type="number" id="wall-angle" value="${(
                    Math.atan2(this.selectedItem.height, this.selectedItem.width) * 180 / Math.PI
                ).toFixed(0)}" min="0" max="360">
            </div>
            <button class="btn btn-primary" id="save-properties">Save</button>
            <button class="btn btn-outline" id="delete-item">Delete</button>
        `;
        ...
        formHTML = `
            <div class="form-group">
                ...
                <select id="decoration-type">
                    <option value="plant" ${this.selectedItem.subtype === 'plant' ? 'selected' : ''}>Plant</option>
                    <option value="bar" ${this.selectedItem.subtype === 'bar' ? 'selected' : ''}>Bar</option>
                    <option value="counter" ${this.selectedItem.subtype === 'counter' ? 'selected' : ''}>Counter</option>
                </select>
            </div>
            <button class="btn btn-primary" id="save-properties">Save</button>
            <button class="btn btn-outline" id="delete-item">Delete</button>
...
saveItemProperties() {
    ...
    if (this.selectedItem.type === 'table') {
        this.selectedItem.number = document.getElementById('table-number').value;
        this.selectedItem.waiter = document.getElementById('table-waiter').value;
        this.selectedItem.width = parseInt(document.getElementById('table-width').value) * 
            (this.selectedItem.width < 0 ? -1 : 1);
        this.selectedItem.height = parseInt(document.getElementById('table-height').value) * 
            (this.selectedItem.height < 0 ? -1 : 1);
    }
    else if (this.selectedItem.type === 'wall') {
        const length = parseInt(document.getElementById('wall-length').value);
        const angle = parseInt(document.getElementById('wall-angle').value) * Math.PI / 180;
        
        this.selectedItem.width = length * Math.cos(angle);
        this.selectedItem.height = length * Math.sin(angle);
    }
    else if (this.selectedItem.type === 'decoration') {
        this.selectedItem.subtype = document.getElementById('decoration-type').value;
    }
    
    this.saveFloorPlan();
    this.renderFloorItems();
    this.showAlert('Properties saved successfully', 'success');
}

deleteSelectedItem() {
    if (!this.selectedItem) return;
    
    this.floorItems = this.floorItems.filter(item => item.id !== this.selectedItem.id);
    this.selectedItem = null;
    this.saveFloorPlan();
    this.renderFloorItems();
    this.renderPropertyForm();
...
addFloorItem(type, subtype) {
    const newItem = {
        ...
        x: 100,
        y: 100,
        width: type === 'table' ? 80 : 100,
        height: type === 'table' ? 80 : 10,
        rotation: 0,
        number: '',
        waiter: ''
    };
    
    this.floorItems.push(newItem);
...
getDecorationIcon(type) {
    switch(type) {
        ...
        case 'counter': return 'utensils';
        default: return 'question';
    }
}

saveFloorPlan() {
    localStorage.setItem('hoteza_floorPlan', JSON.stringify(this.floorItems));
}
...
deleteExpense(index) {
    ...
    // Add these to your generateReport() method if you need those metrics:
...
const lastWeekOrders = this.orders.filter(order => {
    ...
    return orderDate >= sevenDaysAgo && orderDate <= new Date(reportDate);
});
...
this.orders.forEach(order => {
    ...
    if (!allOrdersByDate[date]) allOrdersByDate[date] = 0;
    allOrdersByDate[date] += order.total;
});
let highestDayAmount = 0;
let highestDayDate = '';
Object.entries(allOrdersByDate).forEach(([date, amount]) => {
